...
from utils.state_to_array import state_to_array
from utils.action_to_dict import action_to_dict
...
# 未给定值时的默认值
AREA_OF_NODES = ci.AREA_OF_NODES  # 将节点散落的区域假定为规整矩形
N_OF_NODES = ci.N_OF_NODES  # 所有的IIoT节点，N，其中包括block_producer
N_OF_BLOCK_PRODUCER = ci.N_OF_BLOCK_PRODUCER  # 常选择21，block_producer的个数，暂时将生产者数量固定
AVERAGE_TRANSACTION_SIZE = ci.AVERAGE_TRANSACTION_SIZE  # 200B
# ---------------------------------------------------------
# 权益及算力的分配应当服从高斯分布
# STAKE_OF_NODES = (10, 50)  # 对不同的节点权益分配,范围(0, 1)
STAKE_OF_NODES = ci.STAKE_OF_NODES  # 对不同的节点权益分配,范围(0, 1)
POWER_OF_NODES = ci.POWER_OF_NODES
# ---------------------------------------------------------
COMPUTING_RESOURCE_OF_NODE = ci.COMPUTING_RESOURCE_OF_NODE  # 暂定为10~30GHz，(generating MACs和verifying MACs)
BLOCK_SIZE_LIMIT = ci.BLOCK_SIZE_LIMIT  # 最大区块的大小限制设定为8M，为统一单位定为B
MAX_BLOCK_INTERVAL = ci.MAX_BLOCK_INTERVAL  # 最大区块打包间隔时间10s
ETA_S = ci.ETA_S
ETA_L = ci.ETA_L  # 基尼系数中对去中心化程度的最大限制设定为0.2与0.3
BATCH_SIZE = ci.BATCH_SIZE  # 如在PBFT算法中，一个massage中，primary节点处理的request
...
class BlockChainEnv(gym.Env):
    ...
    def __init__(self, n_of_nodes=N_OF_NODES, area_of_nodes=AREA_OF_NODES,
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                'transaction_size':spaces.Box(low=100,high=300,shape=(1,)),
        ...
        self.action_space = spaces.Dict({
            ...
            'block_interval': spaces.Box(low=0.5, high=MAX_BLOCK_INTERVAL, shape=(1,))
        })
        # -----------------------------------------------------------------------------------------------------
    ...
    def reset(self):
        self.state = self.observation_space.sample()
        return state_to_array(self.state).to_array()
    ...
    def step(self, action):
        ...
        action_dict = action_to_dict(action).to_dict()

        print("第{}次step:".format(self.step_count))
        self.step_count += 1
        ...
        Tl_norm, location, transaction_size, transmission_rate = (s[k] for k in s.keys())
        ...
        no_block_producer = np.count_nonzero(action_dict['no_block_producer'])
        self.index_of_producer = np.argwhere(np.array(action_dict['no_block_producer']) == 1)
        ...
        '''
        coef_of_security = self.get_security_coef(action_dict)

        '''
        根据节点位置及block_producer的改变，transmition_rate改变(10~100Mbps)
        是通过模拟，还是通过计算？
        '''
        # transmission_rate维度不一致？？？？？？？？？？？？？？？6
        transmission_rate, base_trans_rate = self.get_transmission_rate()
        # print(transmission_rate)

        '''
        计算能力，事先对不同的节点赋予不同的计算能力
        '''
        computing_capacity_of_IIoT_nodes, T_final = self.get_computing_capacity(action_dict, transmission_rate)
        ...
        Tl_norm = Theil_instance.get_Theil_norm(action_dict['no_consensus_algorithm'], Tl_norm_C, Tl_norm_G, Tl_norm_L)

        # 单位B/s，计算使用最终达到一致性时间
        # throughout = np.floor(action_dict['block_size'] / self.average_transaction_size) * 1000 / (
        #             action_dict['block_interval'] + T_final)
        throughout = np.floor(action_dict['block_size'] / transaction_size) * 1000 / (
                    action_dict['block_interval'] + T_final)

        s_ = OrderedDict({
            'Tl_norm': np.array(Tl_norm),
            'location_of_producers': np.array(location),
            # 'throughout': np.array(throughout),
            'transaction_size':np.array(transaction_size),
            'transmission_rate': np.array(base_trans_rate),
        })
        

        self.state = s_
        s_ = state_to_array(s_).to_array()
    ...
    def get_transmission_rate(self):
        base_transmission_rate = np.random.rand()*10 + 90  # 利用虚拟机或其他方法模拟节点时间通讯的基本延迟
        ...
        for x in index_of_producer:
            for y in index_of_producer:
                if y != x:
                    transmission[x][y] = base_transmission_rate
        # transmission = np.random.rand()*90 + 10
        return transmission, base_transmission_rate
